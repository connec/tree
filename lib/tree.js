        ...
        ((function() {
            ...
            module.exports = Tree = function() {
                ...
                Tree.prototype.set_root = function(node) {
                    ...
                    if (this.trigger("node:add", node, null) === false) return false;
                    this.$wrapper.append(node.$elem);
                    _ref = [ this.root, node ], old_root = _ref[0], this.root = _ref[1];
                    return old_root;
                };
                ...
                Tree.prototype.insert_node = function(node, context, index) {
                    ...
                    if (!(node instanceof Node)) node = new Node(node);
                    if (this.trigger("node:add", node, context) === false) return false;
                ...
                Tree.prototype.draw_edges = function() {
                    var context, cp1_offset, cp2_offset, end, get_child_offset, node, prepare_context, start, swap, x, y, _i, _len, _ref, _ref2, _ref3, _ref4, _ref5, _results;
                    var _this = this;
                    get_child_offset = function(node) {
                        return (node.$elem[_this.options.breadth]() - parseInt(node.$edge.css("line-height"))) / 2 + node.$elem.position()[_this.options.child_offset];
                    };
                    prepare_context = function(node) {
                        var context;
                    ...
                    swap = function(x, y) {
                        var _ref;
                        if (_this.options.level_offset === "right") {
                            _ref = [ x.end, x.start, x.cp2, x.cp1 ], x.start = _ref[0], x.end = _ref[1], x.cp1 = _ref[2], x.cp2 = _ref[3];
                        }
                        if (_this.options.breadth === "height") return [ y, x ];
                        return [ x, y ];
                    };
                    cp1_offset = .3 * this.options.level_spacing;
                    cp2_offset = this.options.level_spacing - cp1_offset;
                    _ref = this.root.subtree_nodes();
                    _results = [];
                    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                        node = _ref[_i];
                        ...
                        y = {
                            start: 0,
                            end: this.options.level_spacing
                        };
                        context = prepare_context(node);
                        if (start === end) {
                            _ref2 = swap(x, y), x = _ref2[0], y = _ref2[1];
                            context.moveTo(x.start, y.start);
                            context.lineTo(x.end, y.end);
                        } else {
                            _ref3 = [ start, end ], x.cp1 = _ref3[0], x.cp2 = _ref3[1];
                            _ref4 = [ cp1_offset, cp2_offset ], y.cp1 = _ref4[0], y.cp2 = _ref4[1];
                            _ref5 = swap(x, y), x = _ref5[0], y = _ref5[1];
                            context.moveTo(x.start, y.start);
                            context.bezierCurveTo(x.cp1, y.cp1, x.cp2, y.cp2, x.end, y.end);
                        }
                ...
                Tree.prototype.get_child_offset = function(node) {
                    ...
                    return this.use_cache("child_offset", node, function() {
                        ...
                        if (!node.children.length) return offset;
                ...
                Tree.prototype.get_subtree_breadth = function(node) {
                    ...
                    return this.use_cache("subtree_breadth", node, function() {
                        ...
                        if (!node.children.length) return node.$elem[_this.options.breadth]();
                        breadth = _this.options.child_spacing * (node.children.length - 1);
                        _ref = node.children;
                        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
                            child = _ref[_i];
